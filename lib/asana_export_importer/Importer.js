...
var Importer = module.exports = aei.ideal.Proto.extend().setType("Importer").newSlots({
    ...
    _importTags: function() {
        var firstPageCollection = aei.Future.withPromise(this.app().apiClient().tags.findByWorkspace(this.organizationId(), {limit: 500})).wait();
    ...
    _addTasksToColumns: function() {
        this._forEachOfType("column", this._addItemsToObjectForwards, "adding tasks to columns");
    },
    ...
    /**
     * Like _addItemsToObject, but for endpoints which insert at the bottom.
     */
    _addItemsToObjectForwards: function(object) {
        this._addSpecifiedItemsToObject(object, object.sourceItemIds());
    },

    /**
     * Calls addItem, passing the asanaId for each element of sourceItemIds in reverse order.
     * Reverse order, because the majority of Asana API insertions insert at the top.
     */
    _addItemsToObject: function(object) {
        this._addSpecifiedItemsToObject(object, object.sourceItemIds().reverse());
    },

    _addSpecifiedItemsToObject: function(object, sourceItemIds) {
        var self = this;
        sourceItemIds.forEach(function(sourceItemId, index) {
