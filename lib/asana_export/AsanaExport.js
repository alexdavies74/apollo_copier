        ...
        return function(position, chunkSize) {
            return self.db().findByType("Task", position, chunkSize).map(function(obj){
                ...
                if (obj.stories.length > 0) {
                    self.db().findChildrenByType(obj.__object_id, "Comment").forEach(function(story) {
                        storyTexts[story.__object_id] = { creator: Object.perform(self._userForDomainUserId(story.creator_du), "creatorId"), text: story.text };
                    });
                }
                ...
                return ae.aei.Task.clone().performSets({
                    creatorDu: Object.perform(self._userForDomainUserId(obj.creator_du), "creatorId"),
                    sourceId: obj.__object_id,
                    name: obj.name,
